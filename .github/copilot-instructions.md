...
- **Django 4.2** web framework with SQLite database (`db/job_tracker.db`)
...
```python
# Example: Checking ATS domains
if _is_ats_domain(sender_domain):
    company = extract_from_ats_sender(sender_display_name)
```
...
# Export companies.json after manual edits via dashboard
...
```bash
# Start Django dev server (custom runserver command with auto-reload)
...
| `db.py` | Legacy SQLite utilities (being phased out in favor of Django ORM) |
...
- `/` - Main dashboard with thread view
- `/companies/` - Company management and labeling
- `/messages/` - Message review and bulk labeling
- `/admin/json-files/` - Edit patterns.json and companies.json in-browser
...
3. **Classification sources** - Track via `Message.classification_source` (values: `ml`, `rule`, `rules_override`)
...
6. **Django ORM first** - Legacy `db.py` functions are being phased out; prefer Django ORM for new code
