...
### 2. GMAIL_JOBHUNT_LABEL_ID

**What it is:** The unique identifier for your Gmail label that contains job hunting emails.

**Prerequisites:**
- Gmail account
- `credentials.json` file from Google Cloud Console (see below)
...
This will output all your Gmail labels with their IDs:
```
INBOX: INBOX
Sent: SENT
JobHunt: Label_1234567890123456789
Important: IMPORTANT
...
```

Find your job hunting label (e.g., "JobHunt") and copy its ID.

**Method 2: Using Gmail Management Script**

Create a helper script `get_label_id.py`:
...
def list_labels():
    ...
    for label in sorted(labels, key=lambda x: x['name']):
        print(f"{label['name']:30} {label['id']}")
    print('-' * 60)
    print('\nLook for your job hunting label above and copy its ID.')
...
**Method 3: Using Google APIs Explorer**

1. Go to: https://developers.google.com/gmail/api/reference/rest/v1/users.labels/list
2. Click "Try this API" in the right panel
3. Set `userId` to `me`
4. Click "Execute"
5. Authorize with your Google account
6. In the response, find your label and copy the `id` field

**Method 4: Create Label via Script**

If you don't have a label yet, create one:
...
if __name__ == '__main__':
    label_id = create_label('JobHunt')
    print(f"\nAdd this to your .env file:")
    print(f"GMAIL_JOBHUNT_LABEL_ID={label_id}")
```
...
### 3. GMAIL_ROOT_FILTER_LABEL (Optional)
