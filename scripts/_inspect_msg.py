...
print('Message:', msg and msg.id, 'ml_label=', getattr(msg,'ml_label',None), 'confidence=', getattr(msg,'confidence',None))
if msg:
    tt = ThreadTracking.objects.filter(thread_id=msg.thread_id).first()
    print('ThreadTracking:', tt and tt.id, 'ml_label=', getattr(tt,'ml_label',None), 'ml_confidence=', getattr(tt,'ml_confidence',None), 'status=', getattr(tt,'status',None))
else:
