...
from db import COMPANIES_PATH, PATTERNS_PATH, get_db_connection, is_valid_company

parser = argparse.ArgumentParser(
    description="Detect alias candidates from company names"
)
...
def is_clean_company(name):
    ...
    if len(name.split()) > 4:
        return False
    if re.search(
        r"(interview|application|position|received|manager|engineer|researcher|job|role|title|follow-up)",
        name,
        re.I,
    ):
        return False
...
NOISE_TOKEN_RE = re.compile(
    r"(please|sign|attached|thanks|thank you|sincerely|regards|cv|resume|mr|ms|mrs)",
    re.I,
)
...
def is_personal_name_candidate(name):
    ...
    if re.search(
        r"\b(Inc|LLC|Ltd|Corp|Company|Co\.|PLC|Systems|Technologies|Group|Holdings)\b",
        name_stripped,
        re.I,
    ):
        return False
...
def cleaned_candidate(name):
    ...
    s = re.sub(
        r"\b(&\s*the\s+.*Family of Companies|Family of Companies|Family of|Group of Companies|Group of)\b",
        "",
        s,
        flags=re.I,
    )
    s = re.sub(
        r"\b(,?\s*(Inc|LLC|Ltd|Corp|Co\.|Company|PLC|Systems|Technologies|Holdings|Group))\b",
        "",
        s,
        flags=re.I,
    )
    ...
    s = re.sub(
        r"\b(the\s+)?(senior|lead|principal|junior|machine learning|engineer|developer|manager|analyst|recruiter|consultant)\b",
        "",
        s,
        flags=re.I,
    )
    ...
    s = re.sub(
        r"\b(Application|Interview|Update|Confirmation|Availability|Please|Attached|Signature|Sign|Resume|CV)\b",
        "",
        s,
        flags=re.I,
    )
...
def likely_noise(name):
    ...
    if re.search(
        r"\b(senior|lead|principal|junior|machine learning|engineer|developer|manager|analyst|recruiter|intern)\b",
        name,
        re.I,
    ):
        return True
...
if __name__ == "__main__":
    ...
    if candidates.empty:
        ...
        for idx, row in candidates.iterrows():
            ...
            if not best_canon:
                overlap_canon, overlap_score = best_canonical_by_overlap(
                    name, canonical_names, min_overlap=0.35
                )
                if overlap_canon:
                    best_canon = overlap_canon
                    sim_score = (
                        overlap_score  # reuse sim_score field for export/decision
                    )
            ...
            # 3) If canonical exists but sim lower than 0.75 and cleaned is poor, still allow canonical if sim >= 0.6
                ...
                best_canon
                and 0.6 <= sim_score < 0.75
                and best_canon.lower() != name.lower()
            ):
                suggestion = best_canon
            ...
            if (
                not suggestion
                and is_personal_name_candidate(name)
                and "&" not in name
                and re.search(
                    r"\b(Inc|LLC|Ltd|Corp|Co\.|Company|PLC|Systems|Technologies|Group|Holdings)\b",
                    name,
                    re.I,
                )
                is None
            ...
            if suggestion:
                ...
                if DEBUG:
                    print(
                        f'  ✅ Exported: "{name}" → "{suggestion}" | reason={reason} | thread_id={thread_id}'
                    )
