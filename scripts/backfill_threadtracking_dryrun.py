...
def main():
    ...
    qs = Message.objects.filter(ml_label__in=["job_application", "interview_invite"]).order_by("-timestamp")
    for msg in qs:
        ...
        if tt:
            if tt.ml_label != msg.ml_label:
                ...
                if len(examples_update) < 10:
                    examples_update.append({"msg_id": msg.id, "thread_id": msg.thread_id, "msg_label": msg.ml_label, "tt_label": tt.ml_label})
            continue
        if not msg.company:
            continue
        would_create += 1
        if len(examples_create) < 10:
            examples_create.append({"msg_id": msg.id, "thread_id": msg.thread_id, "label": msg.ml_label, "company": msg.company.name if msg.company else None})
