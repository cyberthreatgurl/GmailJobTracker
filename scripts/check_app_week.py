...
for t in all_threads:
    print(f"   - {t.company.name:30} sent={t.sent_date} ml_label={t.ml_label or 'None':20} status={t.company.status}")
...
for t in annotated:
    print(f"   - {t.company.name:30} thread={t.thread_id:20} has_job_app={t.has_job_app}")
...
company_names = with_job_app.values_list('company__name', flat=True).distinct().order_by('company__name')
print(f"\n   Companies counted:")
