    ...
    try:
        ...
        if companies_path.exists():
            with open(companies_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                headhunter_domains = [d.strip().lower() for d in data.get('headhunter_domains', []) if d and isinstance(d,str)]
    except Exception:
        headhunter_domains = []
...
def main():
    ...
    if headhunter_domains:
        ...
        hh_companies = Company.objects.filter(hh_company_q | msg_hh_q | Q(message__ml_label='head_hunter')).distinct().values_list('id', flat=True)
        hh_company_list = list(hh_companies)
    print('hh_company_list sample:', hh_company_list[:10])

    # Build tracked_thread_companies
    tt_with_interview = ThreadTracking.objects.filter(interview_date__isnull=False, company__isnull=False).values('thread_id','company_id')
    tracked = {(item['thread_id'], item['company_id']) for item in tt_with_interview}
    print('tracked thread-company pairs count:', len(tracked))

    # msg interview qs
    msg_interviews_qs = Message.objects.filter(ml_label__in=['interview_invite','interview'], company__isnull=False).select_related('company')
    ...
    for m in msgs:
        print('MSG', m.id, m.thread_id, m.company_id, m.company.name if m.company else None, m.ml_label, m.timestamp)
