...
teams_msgs = Message.objects.filter(body__icontains='Microsoft Teams').exclude(ml_label='noise')
...
for msg in teams_msgs[:20]:  # Show first 20
    ...
    has_join_link = bool(re.search(r'join.*meeting|teams.microsoft.com', msg.body, re.I))
    print(f'Has Meeting ID: {has_meeting_id}, Has Join Link: {has_join_link}')
    print('-' * 80)

print('\n=== Searching for Google Meet/Calendar invites ===')
google_msgs = Message.objects.filter(body__iregex=r'(google meet|meet.google.com|calendar invitation)').exclude(ml_label='noise')
