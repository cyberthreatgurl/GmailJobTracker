...
for msg in job_app_messages:
    ...
    if thread:
        print(
            f"  ThreadTracking: sent_date={thread.sent_date}, ml_label={thread.ml_label}, status={thread.status}"
        )
    else:
...
print(
    "Distinct companies from those messages:",
    job_app_messages.values("company_id").distinct().count(),
)

# Now check ThreadTracking perspective
threads_in_range = (
    ThreadTracking.objects.filter(sent_date__gte=week_cutoff, company__isnull=False)
