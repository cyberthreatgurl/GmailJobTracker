...
for m in qs.order_by('-timestamp')[:50]:
    print(m.id, m.msg_id, m.thread_id, m.timestamp, m.ml_label, m.company.name if m.company else None, (m.subject or '')[:120])

# Also list any ThreadTracking with company SentinelOne
tt = ThreadTracking.objects.filter(company__name__icontains='SentinelOne', interview_date__isnull=False)
