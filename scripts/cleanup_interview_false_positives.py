...
parser = argparse.ArgumentParser(description='Cleanup low-confidence interview_date ThreadTracking rows')
parser.add_argument('--apply', action='store_true', help='Apply changes (default is dry-run)')
parser.add_argument('--threshold', type=float, help='Confidence threshold override (default from env or 0.6)')
...
def main():
    ...
    candidates = ThreadTracking.objects.filter(interview_date__isnull=False, interview_completed=False)
    to_clear = []

    for tt in candidates:
        # If there are message-level interview labels, keep
        msgs = Message.objects.filter(thread_id=tt.thread_id)
        has_msg_interview = msgs.filter(ml_label__in=['interview_invite', 'interview']).exists()
...
if __name__ == '__main__':
    print(f"Running cleanup_interview_false_positives.py (threshold={THRESHOLD}, dry_run={DRY_RUN})")
    main()
