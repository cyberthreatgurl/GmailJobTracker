...
parser.add_argument('--threshold', type=float, default=0.7, help='ML confidence threshold to keep interview_date')
...
candidates = []
for tt in ThreadTracking.objects.filter(interview_date__isnull=False).select_related('company'):
    keep = False
    ...
    if ml_label in ('interview_invite','interview') and ml_conf >= THRESHOLD:
        keep = True
    # message-level check
    has_msg_interview = Message.objects.filter(thread_id=tt.thread_id, ml_label__in=['interview_invite','interview']).exists()
    if has_msg_interview:
        keep = True
    if not keep:
        candidates.append({'id': tt.id, 'thread_id': tt.thread_id, 'company': tt.company.name if tt.company else None, 'ml_label': ml_label, 'ml_confidence': ml_conf, 'interview_date': str(tt.interview_date)})
...
print(f'Found {len(candidates)} ThreadTracking rows that would be cleared (backup written to {backup_path})')
if DRY_RUN:
