...
def find_likely_newsletters(dry_run=True):
    ...
    print(f"Found {len(likely_newsletters)} likely newsletter messages out of {messages.count()} total")
    print()
    ...
    for i, (msg, score, reasons) in enumerate(likely_newsletters, 1):
        try:
            msg_id = msg.msg_id
            print(f"[{i}/{len(likely_newsletters)}] Re-ingesting: {msg.subject[:50]}...")
    ...
    remaining = sum(1 for msg, _, _ in likely_newsletters 
...
if __name__ == "__main__":
    ...
    if execute:
        response = input("⚠️  This will re-ingest likely newsletter messages. Continue? (yes/no): ")
