...
def main():
    ...
    q = ThreadTracking.objects.filter(ml_label="noise").filter(
        Q(rejection_date__isnull=False) | Q(interview_date__isnull=False)
    )
    rows = []
        ...
        rows.append(
            {
                ...
                "rejection_date": (
                    tt.rejection_date.isoformat() if tt.rejection_date else None
                ),
                "interview_date": (
