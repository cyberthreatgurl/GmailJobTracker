...
print(
    f"Headhunter Domains: {', '.join(headhunter_domains) if headhunter_domains else '(none)'}"
)
print(
    f"Date Range: Last 7 days (since {(now() - timedelta(days=7)).strftime('%Y-%m-%d %H:%M')})"
)
...
apps_this_week = applications_qs.filter(timestamp__gte=now() - timedelta(days=7))
...
if apps_this_week.exists():
    ...
    for msg in apps_this_week.order_by("-timestamp")[:10]:
        print(
            f"  • {msg.timestamp.strftime('%Y-%m-%d %H:%M')} | {msg.company.name if msg.company else 'No Company'} | {msg.subject[:60]}"
        )
...
if rejections_qs.exists():
    ...
    for msg in rejections_qs.order_by("-timestamp"):
        print(
            f"  • {msg.timestamp.strftime('%Y-%m-%d %H:%M')} | {msg.company.name if msg.company else 'No Company'} | Label: {msg.ml_label} | {msg.subject[:60]}"
        )
...
thread_interviews_week = ThreadTracking.objects.filter(
    interview_date__gte=(now() - timedelta(days=7)).date(), company__isnull=False
)
...
if thread_interviews_week.exists():
    ...
    for app in thread_interviews_week.order_by("-interview_date"):
        print(
            f"  • {app.interview_date} | {app.company.name} | {app.job_title} | Thread: {app.thread_id}"
        )
...
potential_interviews = (
    Message.objects.filter(
        timestamp__gte=now() - timedelta(days=7), company__isnull=False
    )
    .exclude(ml_label__in=["interview_invite", "noise", "head_hunter"])
    .filter(
...
if potential_interviews.exists():
    ...
    for msg in potential_interviews.order_by("-timestamp")[:10]:
        print(
            f"  • {msg.timestamp.strftime('%Y-%m-%d %H:%M')} | {msg.company.name if msg.company else 'No Company'}"
        )
