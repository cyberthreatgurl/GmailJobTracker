...
print(f'Endyna tracked threads: {[t for t, c_id in tracked_thread_companies if c_id == c.id]}')
...
msg_interviews = Message.objects.filter(
    ...
    company__isnull=False,
    company=c
).select_related('company').order_by('timestamp')
...
print(f'\n=== FINAL COUNT ===')
print(f'ThreadTracking: {len([x for x in interview_companies if x["source"] == "ThreadTracking"])}')
...
for item in sorted(interview_companies, key=lambda x: x['interview_date']):
    print(f'{item["interview_date"]} - {item["source"]} - thread {item["thread_id"][:16]}')
...
in_range = [x for x in interview_companies if datetime.strptime(x['interview_date'], '%Y-%m-%d').date() >= days_90_ago]
