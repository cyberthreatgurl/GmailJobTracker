...
def main():
    ...
    print(json.dumps({'domain_to_company': all_map, 'known': sorted(all_known)}, indent=2))
    
    if args.update and all_map:
        with open(COMPANIES_JSON) as f:
            companies = json.load(f)
        companies.setdefault('domain_to_company', {}).update(all_map)
        companies.setdefault('known', []).extend([k for k in all_known if k not in companies['known']])
