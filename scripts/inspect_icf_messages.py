...
for m in msgs:
    ...
    print(
        f"MSG: {m.id} thread_id:{m.thread_id} ts:{m.timestamp} ml_label:{m.ml_label} ml_conf:{getattr(m,'ml_confidence',None)} sender:{m.sender} subj:{subj}"
    )
...
for tt in tts:
    print(
        f"TT: id:{tt.id} thread_id:{tt.thread_id} sent:{tt.sent_date} interview_date:{tt.interview_date} ml_label:{tt.ml_label} ml_conf:{getattr(tt,'ml_confidence',None)} status:{tt.status} source:{tt.company_source}"
    )
