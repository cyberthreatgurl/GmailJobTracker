...
def show_company(name):
    ...
    for tt in tt_qs[:10]:
        print(
            f"  TT: thread_id={tt.thread_id}, interview_date={tt.interview_date}, job_title={tt.job_title}, interview_completed={tt.interview_completed}"
        )
        print(
            f"    sent_date={tt.sent_date}, rejection_date={tt.rejection_date}, ml_label={tt.ml_label}, ml_confidence={tt.ml_confidence}, status={tt.status}, company_source={tt.company_source}"
        )
    ...
    msg_qs = Message.objects.filter(
        company=comp, ml_label__in=["interview_invite", "interview"]
    ).order_by("-timestamp")
    ...
    for m in msg_qs[:15]:
        subj = (m.subject or "").replace("\n", " ").strip()[:140]
        print(
            f"  MSG: id={m.msg_id}, thread_id={m.thread_id}, ts={m.timestamp}, ml_label={m.ml_label}, sender={m.sender}, subject={subj}"
        )
