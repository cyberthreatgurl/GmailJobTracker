...
def main(sender_email: str, apply_changes: bool = True):
    ...
    qs = Message.objects.filter(sender__icontains=sender_email).exclude(ml_label='noise')
    total = qs.count()
    ...
    backup_path = scripts_dir / f'mark_sender_noise_backup_{sender_email.replace("@","_at_")}_{ts}.json'
    ...
    for tid in thread_ids:
        try:
            ...
            non_noise_exists = Message.objects.filter(thread_id=tid).exclude(ml_label='noise').exists()
    ...
    print(f'Updated {updated_msg} Message(s) and {updated_tt} ThreadTracking(s) for sender {sender_email}')
