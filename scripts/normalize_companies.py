...
def plan_and_apply(apply=False, preserve_ids=None):
    ...
    report = {"timestamp": datetime.utcnow().isoformat() + "Z", "groups": []}
    ...
    for key, group in groups.items():
        ...
        if apply:
            for other in others:
                ...
                print(
                    f"Reassigning Company id={other.id} -> canonical id={canonical.id}"
                )
...
def main():
    ...
    parser.add_argument(
        "--preserve", help="Comma-separated Company ids to preserve from deletion"
    )
    ...
    if not report["groups"]:
        ...
        for g in report["groups"]:
            print(
                f"- {g['key']}: canonical={g['canonical']['name']} ({g['canonical']['id']})"
            )
            for o in g["others"]:
                p = " (preserved)" if o["preserve"] else ""
                print(
                    f"    - {o['name']} (id={o['id']}) msgs={o['messages']} tt={o['threadtracking']}{p}"
                )
