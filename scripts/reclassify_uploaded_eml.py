...
def reclassify_eml_text(raw_text: str) -> dict:
    ...
    tt = ThreadTracking.objects.filter(thread_id=msg.thread_id).first() if msg.thread_id else None
    if tt and ml_label:
    ...
    return {"found": True, "message_id": msg.id, "ml_label": ml_label, "ml_confidence": ml_conf}
