...
if msg.thread_id:
    ...
    if not tt:
        ...
        tt.save()
        print(
            f"Updated ThreadTracking {tt.id}: ml_label={tt.ml_label} ml_confidence={tt.ml_confidence} status={tt.status} rejection_date={tt.rejection_date} interview_date={tt.interview_date}"
        )
else:
