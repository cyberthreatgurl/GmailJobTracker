...
def restore_reply_labels():
    ...
    print(f"📧 Found {len(replies_forwards)} user replies/forwards with incorrect 'other' label")
    ...
    for i, msg in enumerate(replies_forwards, 1):
        try:
            ...
            if result:
                ...
                msg.refresh_from_db()
                print(f"✅ [{i}/{len(replies_forwards)}] Restored: {msg.subject[:60]} -> Label: {msg.ml_label}")
            else:
                skipped += 1
