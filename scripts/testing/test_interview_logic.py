...
print(f'\n=== Final Result ===')
print(f'ThreadTracking entries: {len([x for x in interview_companies if x["source"] == "ThreadTracking"])}')
...
for item in sorted(interview_companies, key=lambda x: x['interview_date']):
    print(f'  {item["interview_date"]} - {item["source"]} - thread={item["thread_id"][:10]}...')
