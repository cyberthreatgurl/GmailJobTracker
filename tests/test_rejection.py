...
body_with_header = (
    """X-LinkedIn-Template: email_jobs_application_rejected_01
"""
    + body
)
...
print("=" * 80)
result2 = parse_subject(
    subject, body_with_header, sender=sender, sender_domain="linkedin.com"
)
print(f"\nResult: {result2}")
...
print("=" * 80)
print(
    f"Test 1 - LinkedIn Rejection: {'✅ PASS' if result1.get('label') == 'rejection' else '❌ FAIL'}"
)
print(
    f"Test 2 - LinkedIn Rejection (with header): {'✅ PASS' if result2.get('label') == 'rejection' else '❌ FAIL'}"
)
print(
    f"Test 3 - Normal Application: {'✅ PASS' if result3.get('label') == 'job_application' else '❌ FAIL'}"
)
