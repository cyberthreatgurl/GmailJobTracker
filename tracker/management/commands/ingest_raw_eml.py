...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--show-body", action="store_true", help="Print first 800 chars of body"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **opts):
        ...
        parsed = (
            parse_subject(subject, body, sender=sender, sender_domain=sender_domain)
            or {}
        )
        ...
        ml_conf = (
            float(ml.get("confidence", ml.get("proba", 0.0)))
            if isinstance(ml, dict)
            else 0.0
        )
        header_hints = meta.get("header_hints", {})
        synthetic_id = hashlib.md5(
            (subject + sender + str(meta.get("date"))).encode()
        ).hexdigest()[:16]
        result = {
