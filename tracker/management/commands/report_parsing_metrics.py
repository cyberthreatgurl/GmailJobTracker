...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        manual = Message.objects.filter(
            company_source__in=["manual_fix", "normalized"]
        ).count()
        ...
        for conf in Message.objects.exclude(confidence=None).values_list(
        ...
        top_companies = Counter(
            Message.objects.exclude(company=None).values_list(
